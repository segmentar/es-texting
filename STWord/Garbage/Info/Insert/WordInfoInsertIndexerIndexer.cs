...
namespace Core.Shared
{
    ...
    {
        public static void Info_WordInsertIndexerIndexer(STSearch search, Int32 start, Int32 length, Boolean debug)
        {
            goto descriptor;
            ...
            descriptor:
            {
                if (SAPolicy.WordInfoPolicy is true)
                {
                    var message = String.Join('\n'.ToString(), new String[] {
                        String.Empty + nameof(STWord) + ' ' + "::" + ' ' + nameof(Info_WordInsertIndexerIndexer) + ' ' + '{',
